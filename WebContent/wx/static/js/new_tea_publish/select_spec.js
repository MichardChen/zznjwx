+(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == REQUEST_OK){
					...
					mui(".mui-numbox").numbox().setOption("max",teaDescData.stock);
				}else{
	...
	var createDom = function(data){
		...
		var html = '<div class="tea-name"><p>'+data.name+'</p><p class="tea-price mui-pull-right">￥'+data.price+'</p></div>'
				 + '<div class="tea-tags"><span class="tag">'+data.wareHouse+'</span><span class="tag">'+data.type+'</span><span class="mui-pull-right">'+data.stock+data.size+'</span></div>'
		$('.tea-text').html(html);
